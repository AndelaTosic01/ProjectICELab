...
namespace MIntermediateRepresentation.RVSDG.Expression
{
    public class Max : Operation
    {
        ...
        public Max(IOutput[] output, IInput[] input) : base(output, input)
        {
        ...
        public override void CalcResult()
        {
            ...
            {
                var maxValue = IntegerType.CastValue(Inputs[0].GetValue());
                for (int i=1; i<Inputs.Count; i++)
                {
                    if (IntegerType.CastValue(Inputs[i].GetValue()) > maxValue)
                        maxValue = IntegerType.CastValue(Inputs[i].GetValue());
                }
                Output[0].SetValue(maxValue);
            }
            else if (type is RealType)
            {
                var maxValue = RealType.CastValue(Inputs[0].GetValue());
                for (int i = 1; i < Inputs.Count; i++)
                {
                    if (RealType.CastValue(Inputs[i].GetValue()) > maxValue)
                        maxValue = RealType.CastValue(Inputs[i].GetValue());
                }
                Output[0].SetValue(maxValue);
            }
        }
