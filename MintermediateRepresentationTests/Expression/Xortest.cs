...
public class Xortest
{
    ...
    {
        Assert.ThrowsException<NullReferenceException>(() => new Xor(output, inputs));
    }
    [TestMethod]
    [DynamicData(nameof(WrongInputParams))]
    public void TestInputConstructor(IOutput[] output, IInput[] inputs)
    {
        Assert.ThrowsException<IrregularInputException>(() => new Xor(output, inputs));
    }
    [TestMethod]
    [DynamicData(nameof(WrongOutputParams))]
    public void TestOutputConstructor(IOutput[] output, IInput[] inputs)
    {
        Assert.ThrowsException<IrregularOutputException>(() => new Xor(output, inputs));
    }
    ...
    {
        Operation op = new Xor(output, inputs);
        op.CalcResult();
